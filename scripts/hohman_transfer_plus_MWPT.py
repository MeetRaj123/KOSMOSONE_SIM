...
def vnb_to_inertial(delta_v_vnb, velocity_vector, position_vector):
    """
    Convert Delta-V from VNB to inertial frame.
    delta_v_vnb: np.array([V, N, B]) in km/s
    velocity_vector: np.array([Vx, Vy, Vz]) in km/s
    position_vector: np.array([Rx, Ry, Rz]) in km
    """
    # Normalize velocity to get V_hat
    v_hat = velocity_vector / np.linalg.norm(velocity_vector)
    
    # Calculate B_hat (angular momentum direction)
    h_vector = np.cross(position_vector, velocity_vector)
    b_hat = h_vector / np.linalg.norm(h_vector)
    
    # Calculate N_hat (normal to V and B)
    n_hat = np.cross(b_hat, v_hat)
    
    # VNB to inertial rotation matrix
    R_N_VNB = np.column_stack((v_hat, n_hat, b_hat))
    
    # Delta-V in inertial frame
    delta_v_inertial = R_N_VNB @ delta_v_vnb
    return delta_v_inertial
...
first_DeltaV = np.array([1.7458742232009,0,0])
second_DeltaV = np.array([-0.751754096160,0,0])
...
def run(show_plots, maneuverCase):
    ...
    delta_v_vnb = np.array([0.4458690932488, 0.0, 0.0])  # Retrograde burn in VNB
    delta_v_vnb1 = np.array([0.4701699886350, 0.0, 0.0])
    velocity_vector = velRef.getState()  # Example from GMAT or Basilisk state logs
    position_vector = posRef.getState()  # Same as above

    position_vector = (np.array(position_vector).flatten())
    velocity_vector = (np.array(velocity_vector).flatten()) #in Km
    print( f"posi : {position_vector} \n velo : {velocity_vector} ")

    inertial_delta_v = vnb_to_inertial(delta_v_vnb, velocity_vector, position_vector)
    print("Delta-V in inertial frame:", inertial_delta_v)
    inertial_delta_v1 = vnb_to_inertial(delta_v_vnb1, velocity_vector, position_vector)
    print("Delta-V in inertial frame:", inertial_delta_v1)
...
# Total simulation time should be long enough to reach apogee 
    ...
    # Check perigee
        burn1_done = check_and_apply_deltaV(scSim, velRef, posRef, r_perigee, first_DeltaV, burn1_done, "Perigee")
    ...
    if burn1_done and not burn2_done:
        print(burn2_done)
        burn2_done = check_and_apply_deltaV(scSim, velRef, posRef, r_appogee2, second_DeltaV, burn2_done, "Apogee")
        print(burn2_done)
    ...
    scSim.ExecuteSimulation()
    print(burn2_done)
    print(f"position initial : {posRef.getState()} \n Velocity initial : {velRef.getState()}")
